...
router.get('/:id',isAuthenticated, async (req, res) => {
    ...
    try {       // Fetch student details
        ...
        res.render('./profile_pages/student_profile.ejs',{ studentDetails, examAttempts,size });
    } catch (error) {
        console.error(error);
                    ...
                    period: {
                        $concat: [
                            ...
                            "-",
                            {
                                $switch: {
                                    branches: [
                                        { case: { $eq: ["$_id.quarter", 1] }, then: "Q1" },
                                        { case: { $eq: ["$_id.quarter", 2] }, then: "Q2" },
                                        { case: { $eq: ["$_id.quarter", 3] }, then: "Q3" },
                                        { case: { $eq: ["$_id.quarter", 4] }, then: "Q4" }
                                    ]
                                }
                            }
                        ]
